...
class ConnectionInterfaceStreamedMedia(dbus.service.Interface):
    """
    For connections which support a concept of streamed media (the streamed
    media channel type is available), the connection manager requires
    knowledge of what codecs are implemented by the clients currently
    available, for initialising channels and responding to invitations.
    Referred to in this specification as the 'user' media parameters,
    this static information is represented by a string of SDP (see IETF
    RFC 2327) which can be set and retrieved using this interface.
    """
    def __init__(self):
        self._interfaces.add(CONN_INTERFACE_STREAMED_MEDIA)
        self._media_parameters = ''

    @dbus.service.method(CONN_INTERFACE_STREAMED_MEDIA, in_signature='s', out_signature='')
    def SetMediaParameters(self, parameters):
        """
        Indicate the codecs available on this connection by providing SDP
        representing the local media parameters.

        Parameters:
        parameters - a string of SDP representing the available local codecs

        Potential Errors:
        InvalidArgument (the provided information is malformed), Disconnected, NetworkError
        """
        self._media_parameters = parameters

    @dbus.service.method(CONN_INTERFACE_STREAMED_MEDIA, in_signature='', out_signature='s')
    def GetMediaParameters(self):
        """
        Retrieve the SDP representing the local media parameters registered
        to this connection.

        Returns:
        a string of SDP representing the available local codecs
        """
        return self._media_parameters
